...
public class DecryptTests {
    ...
    void DecryptSingleBlocksTest() {
        ...
        cipher.init(new AEADParams(HexUtils.hexToBytes(key), HexUtils.hexToBytes(iv), CipherMode.DECRYPT));

        cipher.updateAAD(HexUtils.hexToBytes(aad));

        byte[] block = HexUtils.hexToBytes(ct);

        cipher.processBlock(block);
        assertEquals(pt, HexUtils.bytesToHex(block));
    ...
    void DecryptMultiBlockTest() {
        ...
        byte[] block = HexUtils.hexToBytes(ct.substring(0,32));
        cipher.processBlock(block);
        assertEquals(pt.substring(0,32), HexUtils.bytesToHex(block));

        block = HexUtils.hexToBytes(ct.substring(32,64));
        cipher.processBlock(block);
        assertEquals(pt.substring(32,64), HexUtils.bytesToHex(block));
    ...
    @Test
    void DecryptMultiPTPartialAADBlockTest() {
        ...
        for (int i = 0; i < ct.length(); i+=32) {
            int endIndex = Math.min(i + 32, ct.length());
            byte[] block = HexUtils.hexToBytes(ct.substring(i, endIndex));
            cipher.processBlock(block);
            assertEquals(pt.substring(i, endIndex), HexUtils.bytesToHex(block));
        }
    ...
    @Test
    void DecryptMultiThenPartialBlockTest() {
